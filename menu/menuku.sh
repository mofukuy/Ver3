...
Name=$(curl -sS https://raw.githubusercontent.com/mofukuy/permission/main/ipmini | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/mofukuy/permission/main/ipmini | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/mofukuy/permission/main/ipmini | grep $MYIP | awk '{print $3}')
fi
...
export sem=$( curl -s https://raw.githubusercontent.com/mofukuy/permission/main/versions)
...
echo -e "${CYAN}│\033[0m ${BOLD}${ORANGE}SSH   VMESS    VLESS     TROJAN    SHADOWSOCKS$NC  $COLOR1"

echo -e "${CYAN}│\033[0m ${Blue} $ssh1      $vma        $vla         $tra            $ssa   $NC"
...
9) clear ; genssl ;;
10) clear ; jam ;;
6969) clear ; wget https://raw.githubusercontent.com/mofukuy/Ver3/main/update.sh && chmod +x update.sh && ./update.sh && rm -f /root/update.sh ;;
0) clear ; menu ;;
x) exit ;;
